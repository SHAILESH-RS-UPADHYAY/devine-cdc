...
export default function AboutPage() {
  return (
    <main className="pt-32 pb-16 min-h-screen bg-gray-50">
      ...
      <section className="container mx-auto px-4 md:px-6 mb-20">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <h2 className="text-3xl font-heading font-bold text-devine-purple mb-6">Our Vision</h2>
      ...
      {/* Trust Stats */}
      <section className="bg-devine-purple text-white py-16 mb-20">
              ...
              return (
                <div key={i} className="flex flex-col items-center">
                  <div className="bg-white/10 p-4 rounded-full mb-4">
                    <Icon className="w-8 h-8 text-devine-pink" />
                  </div>
        ...
        <div className="flex flex-col gap-12 max-w-7xl mx-auto">
          {FOUNDERS.map((founder, i) => (
            <div key={i} className="relative group">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-devine-pink via-devine-purple to-devine-blue rounded-[3rem] blur-xl opacity-30 group-hover:opacity-60 transition-opacity duration-500"></div>
              ...
              <Card className="relative flex flex-col md:flex-row overflow-hidden border-2 border-white/50 shadow-[0_20px_50px_rgba(0,0,0,0.1)] hover:shadow-[0_20px_50px_rgba(93,63,211,0.2)] transition-all duration-500 rounded-[3rem] bg-white/80 backdrop-blur-xl">
                ...
                <CardContent className="p-8 md:p-12 md:w-2/3 flex flex-col justify-center relative">
                  ...
                  <p className="text-xl text-devine-purple font-bold mb-6">{founder.role}</p>
                  
                  <div className="w-16 h-1.5 bg-gradient-to-r from-devine-pink to-devine-yellow rounded-full mb-6"></div>
                  ...
                  {founder.credentials.length > 0 && (
                    <div className="grid sm:grid-cols-2 gap-4">
                      {founder.credentials.map((cred, idx) => (
                        <div key={idx} className="flex items-center text-sm font-semibold text-slate-800 bg-devine-purple/5 p-3 rounded-xl border border-devine-purple/10">
                          <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center mr-3 shadow-sm shrink-0">
                            <ShieldCheck className="w-4 h-4 text-devine-pink" />
                          </div>
                          <span>{cred}</span>
                        </div>
                      ))}
